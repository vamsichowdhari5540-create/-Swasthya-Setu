...
**Git auto-deploy is only connected for the mobile web project, deliberately not the admin one.** Both Vercel projects watch the same GitHub repo, but only one `vercel.json` is committed at the root (the mobile app's) — so if the admin project's Git connection were left on, the next ordinary push would silently rebuild the *mobile* app onto the admin URL, overwriting a correct manual deploy with a wrong one, with no error to notice it by. The admin project's Framework Settings are still generic defaults (`Root Directory: .`, no build-command override) — nothing from the manual `vercel.json`-swap deploy persists at the project level. Until the admin project has its own persistent Root Directory (`apps/admin`) and build command configured in the Vercel dashboard — untested here, so don't assume the exact field values — keep deploying it manually via the swap procedure above, not via git push.
