...
**Admin console deploys as a separate Vercel project** (`swasthya-sethu-admin`) from the same repo, since it needs a different build command and output directory than the mobile app. Both projects share one repo and one `vercel.json`, but `vercel.json`'s `buildCommand`/`outputDirectory` — if present — always wins over whatever's configured in a project's own dashboard settings for that deployment. So `vercel.json` at the repo root deliberately carries **no build config**, only the SPA rewrite rule; each project's actual build command and output directory live entirely in its own Vercel dashboard (Settings → Build and Deployment → Framework Settings, both fields toggled to "Override"):

| Project | Build Command | Output Directory |
| --- | --- | --- |
| `swasthya-sethu` (mobile) | `npm run build:web` | `apps/mobile/dist` |
| `swasthya-sethu-admin` | `npm run build:admin` | `apps/admin/dist` |

With that split, git auto-deploy is safely connected for both projects — a push rebuilds each from its own dashboard settings, and neither can silently overwrite the other's output the way it could when `vercel.json` carried a build command.
