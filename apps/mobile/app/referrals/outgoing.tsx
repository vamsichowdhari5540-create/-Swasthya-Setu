...
export default function OutgoingReferralsScreen() {
  ...
  const load = useCallback(() => {
    apiFetch<Referral[]>(session, '/api/referrals?box=outgoing').then(setReferrals);
  }, [session]);
        ...
        renderItem={({ item }) => (
          <Link href={`/referrals/${item.id}`} asChild>
            <Pressable
              ...
              <Text style={[styles.meta, { color: colors.muted }]}>To {item.receivingFacilityName}</Text>
